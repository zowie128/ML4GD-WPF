...
class ProductGraph:
    ...
    def generate_Kronecker(self):
        return torch.tensor(np.kron(self.S_T, self.S))

    def generate_Cartesian(self):
        I_N = np.eye(self.S.shape[0])
        I_T = np.eye(self.S_T.shape[0])
        return torch.tensor(np.kron(self.S_T, I_N) + np.kron(I_T, self.S))
    ...
    def generate_parametric(self):
        I_T = np.eye(self.S_T.shape[0])
        I_N = np.eye(self.S.shape[0])
        return torch.tensor(np.kron(I_T, I_N)) + self.generate_Strong()